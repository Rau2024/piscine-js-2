...
export const explore = () => {
  const orderedDestinations = sort()
  const docBody = document.body

  orderedDestinations.map(location => {
    const locationSection = document.createElement('section')
    const cityName = location.name.split(',')[0].toLowerCase().replaceAll(' ', '-')
    locationSection.style.background = `url('./where-do-we-go_images/${cityName}.jpg')`
    docBody.append(locationSection)
  })

  const locationLink = document.createElement('a')
  locationLink.classList.add('location')
  locationLink.textContent = `${orderedDestinations[0].name}\n${orderedDestinations[0].coordinates}`
  locationLink.href = `https://www.google.com/maps/place/${orderedDestinations[0].coordinates}`
  locationLink.style.color = orderedDestinations[0].color
  docBody.append(locationLink)

  const compassElement = document.createElement('div')
  compassElement.classList.add('direction')
  docBody.append(compassElement)
  let previousScroll = 0
  let currentScroll = 0

  document.addEventListener("scroll", function () {
    currentScroll = window.scrollY
    currentScroll > previousScroll ? compassElement.textContent = "S" : compassElement.textContent = "N"
    let activeIndex = Math.round(window.scrollY / window.innerHeight)
    locationLink.href = `https://www.google.com/maps/place/${orderedDestinations[activeIndex].coordinates}`
    locationLink.target = "_blank"
    locationLink.style.color = orderedDestinations[activeIndex].color
    locationLink.textContent = `${orderedDestinations[activeIndex].name}\n${orderedDestinations[activeIndex].coordinates}`
    previousScroll = window.scrollY
  })
}

const sort = () => {
  const northernDestinations = places.filter(location => location.coordinates.includes("N"))
  const southernDestinations = places.filter(location => location.coordinates.includes("S"))
  northernDestinations.sort((a, b) => {
    if (a.coordinates > b.coordinates) return -1
    if (b.coordinates > a.coordinates) return 1
  })
  southernDestinations.sort((b, a) => {
    if (a.coordinates > b.coordinates) return -1
    if (b.coordinates > a.coordinates) return 1
  })
  return northernDestinations.concat(southernDestinations)
}
